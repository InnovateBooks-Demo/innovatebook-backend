  ...
  const fetchLeads = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const inProgress = data.leads.filter((l) =>
          ["Intake", "Feasibility Check", "Costing"].includes(l.status),
        ).length;
        const approved = data.leads.filter(
          (l) => l.status === "Approved",
        ).length;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Global Manufacturing Lead Management System
              </p>
            </div>
                    ...
                    filteredLeads.map((lead) => (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="font-medium text-[#033F99]">
                            {lead.lead_id}
                          </div>
                          {lead.rfq_number && (
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-900">
                            {lead.product_description?.substring(0, 50)}...
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeColor(lead.status)}`}
                          >
                            {lead.status}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getPriorityBadgeColor(lead.priority)}`}
                          >
                            {lead.priority}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <button
                            onClick={() =>
                              navigate(
                                `/commerce/manufacturing-leads/${lead.lead_id}`,
                              )
                            }
                            className="text-[#033F99] hover:text-blue-900 font-medium"
                          >
                            View Details
                          </button>
                        </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="bg-gray-100 rounded-lg p-4">
              <h3 className="font-bold text-gray-700 mb-4">
                Intake (
                {leads.filter((l) => l.current_stage === "Intake").length})
              </h3>
              {leads
                .filter((l) => l.current_stage === "Intake")
                .map((lead) => (
                  <div
                    key={lead.id}
                    className="bg-white p-4 rounded-lg shadow mb-3 cursor-pointer hover:shadow-lg transition-shadow"
                    onClick={() =>
                      navigate(`/commerce/manufacturing-leads/${lead.lead_id}`)
                    }
                  >
                    <div className="font-medium text-gray-900">
                      {lead.lead_id}
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {lead.product_description?.substring(0, 40)}...
                    </div>
                  </div>
                ))}
            </div>
            ...
            <div className="bg-gray-100 rounded-lg p-4">
              <h3 className="font-bold text-gray-700 mb-4">
                Feasibility (
                {leads.filter((l) => l.current_stage === "Feasibility").length})
              </h3>
              {leads
                .filter((l) => l.current_stage === "Feasibility")
                .map((lead) => (
                  <div
                    key={lead.id}
                    className="bg-white p-4 rounded-lg shadow mb-3 cursor-pointer hover:shadow-lg transition-shadow"
            ...
            <div className="bg-gray-100 rounded-lg p-4">
              <h3 className="font-bold text-gray-700 mb-4">
                Costing (
                {leads.filter((l) => l.current_stage === "Costing").length})
              </h3>
              {leads
                .filter((l) => l.current_stage === "Costing")
                .map((lead) => (
                  <div
                    key={lead.id}
                    className="bg-white p-4 rounded-lg shadow mb-3 cursor-pointer hover:shadow-lg transition-shadow"
            ...
            <div className="bg-gray-100 rounded-lg p-4">
              <h3 className="font-bold text-gray-700 mb-4">
                Approval (
                {leads.filter((l) => l.current_stage === "Approval").length})
              </h3>
              {leads
                .filter((l) => l.current_stage === "Approval")
                .map((lead) => (
                  <div
                    key={lead.id}
                    className="bg-white p-4 rounded-lg shadow mb-3 cursor-pointer hover:shadow-lg transition-shadow"
