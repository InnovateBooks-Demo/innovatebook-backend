...
const TasksList = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const endpoint = isMyTasks
        ? "/api/operations/tasks/my"
  ...
  const updateTaskStatus = async (taskId, newStatus, reason = null) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/operations/tasks/${taskId}/status`,
        {
          method: "PUT",
          headers: {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Atomic execution & workflow orchestration
                </p>
              </div>
            </div>
                ...
                {filteredTasks.map((task) => (
                  <tr key={task.task_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium capitalize ${getPriorityBadge(task.priority)}`}
                      >
                        {task.priority}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${getStatusBadge(task.status)}`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {task.status === "blocked" && (
                          <button
                            onClick={() =>
                              updateTaskStatus(task.task_id, "ready")
                            }
                ...
                <div className="space-y-3">
                  {groupedTasks[status]?.map((task) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${getPriorityBadge(task.priority)}`}
                        >
                          {task.priority}
                        </span>
