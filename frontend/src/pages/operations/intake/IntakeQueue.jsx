...
const IntakeQueue = () => {
  ...
  const handleAccept = async (workOrderId) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/operations/work-intake/${workOrderId}/accept`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleBlock = async (workOrderId) => {
    ...
    try {
      const token = authService.getToken();
      const response = await fetch(
        `${API_URL}/api/operations/work-intake/${workOrderId}/block`,
        {
          method: "PUT",
          headers: {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-blue-100 rounded-xl flex items-center justify-center">
                <ClipboardList className="h-7 w-7 text-[#033F99]" />
              </div>
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Receive and validate work orders from contracts
                </p>
              </div>
            </div>
          </div>
              ...
              {filteredOrders.map((wo) => (
                <tr key={wo.work_order_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">
                    {wo.sla_snapshot?.duration_days
                      ? `${wo.sla_snapshot.duration_days} days`
                      : "-"}
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${getStatusBadge(wo.status)}`}
                    >
                      {wo.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() =>
                          navigate(`/operations/intake/${wo.work_order_id}`)
                        }
