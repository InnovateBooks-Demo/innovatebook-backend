  ...
  const tabs = [
    {
      ...
      path: "/intelligence/forecasts/revenue",
    },
    {
      id: "spend",
      label: "Spend Forecast",
      path: "/intelligence/forecasts/spend",
    },
    {
      id: "cashflow",
      label: "Cash Flow Forecast",
      path: "/intelligence/forecasts/cashflow",
    },
  ];

  const activeTab =
    tabs.find((t) => location.pathname === t.path)?.id || "revenue";
              ...
              {revenueData.quarters.map((q, i) => (
                <div key={i} className="flex-1 flex flex-col items-center">
                  <div className="w-full flex items-end justify-center gap-2 h-48">
                    <div
                      className="w-12 bg-orange-500 rounded-t relative"
                      style={{ height: `${q.forecast * 3}px` }}
                    >
              ...
              {spendData.categories.map((cat, i) => (
                <div key={i}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-gray-500">
                      ₹{cat.current}M → ₹{cat.forecast}M
                    </span>
                  </div>
                  <div className="h-4 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-orange-400 to-orange-600 rounded-full"
                      style={{ width: `${(cat.forecast / 50) * 100}%` }}
                    ></div>
                  </div>
                </div>
              ))}
                ...
                <div key={i} className="flex-1 flex flex-col items-center">
                  <div className="w-full flex items-end justify-center gap-1 h-48">
                    <div
                      ...
                      style={{ height: `${m.inflow * 4}px` }}
                    ></div>
                    <div
                      className="w-8 bg-red-400 rounded-t"
                      style={{ height: `${m.outflow * 4}px` }}
                    ></div>
                  </div>
