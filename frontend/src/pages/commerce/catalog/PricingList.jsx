...
const PricingList = () => {
  ...
  const filteredPricing = pricing.filter((item) => {
    const matchesSearch =
      ...
      item.pricing_id?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus =
  ...
  return (
    <div className="min-h-screen bg-gray-50" data-testid="pricing-list">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Manage pricing tiers and discount structures
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => navigate("/commerce/catalog/pricing/create")}
      ...
      <div className="px-8 py-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              ...
              <div className="h-12 w-12 rounded-lg bg-[rgba(3,63,153,0.08)] flex items-center justify-center">
                <DollarSign className="h-6 w-6 text-[#033F99]" />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-semibold text-gray-900">
                  {pricing.filter((p) => p.pricing_type === "discount").length}
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">Tier Rules</p>
                <p className="mt-2 text-3xl font-semibold text-gray-900">
                  {pricing.filter((p) => p.pricing_type === "tier").length}
                </p>
              </div>
              ...
              {filteredPricing.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center">
                    ...
                    <button
                      onClick={() =>
                        navigate("/commerce/catalog/pricing/create")
                      }
                ...
                filteredPricing.map((item) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() =>
                        navigate(`/commerce/catalog/pricing/${item.pricing_id}`)
                      }
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm text-gray-600">
                          {item.valid_from || "Always"} -{" "}
                          {item.valid_to || "No end"}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span
                      ...
                      >
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() =>
                              navigate(
                                `/commerce/catalog/pricing/${item.pricing_id}`,
                              )
                            }
                          ...
                          <button
                            onClick={() =>
                              navigate(
                                `/commerce/catalog/pricing/${item.pricing_id}/edit`,
                              )
                            }
                            className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg"
                          >
        ...
        {filteredPricing.length > 0 && (
          <div className="mt-4 flex items-center justify-between">
            <p className="text-sm text-gray-500">
              Showing{" "}
              <span className="font-medium">{filteredPricing.length}</span>{" "}
              pricing rules
            </p>
          </div>
        )}
