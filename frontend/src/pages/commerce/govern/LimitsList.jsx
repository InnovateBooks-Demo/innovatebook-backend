...
const LimitsList = () => {
  ...
  const filteredLimits = limits.filter((item) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50" data-testid="limits-list">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Manage spending and operational limits
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => navigate("/commerce/govern/limits/create")}
      ...
      <div className="px-8 py-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              ...
              <div className="h-12 w-12 rounded-lg bg-[rgba(3,63,153,0.08)] flex items-center justify-center">
                <Gauge className="h-6 w-6 text-[#033F99]" />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-semibold text-gray-900">
                  {limits.filter((l) => l.status === "active").length}
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">Exceeded</p>
                <p className="mt-2 text-3xl font-semibold text-gray-900">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-semibold text-gray-900">
                  {limits.filter((l) => l.status === "inactive").length}
                </p>
              </div>
                ...
                filteredLimits.map((item) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() =>
                        navigate(`/commerce/govern/limits/${item.limit_id}`)
                      }
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 bg-gray-100 text-gray-700 rounded-lg text-xs font-medium capitalize">
                          {item.limit_type || "spending"}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <p className="text-sm font-semibold text-gray-900">
                          ₹{(item.threshold || 0).toLocaleString()}
                        </p>
                      </td>
                      <td className="px-6 py-4 text-right">
                      ...
                      >
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() =>
                              navigate(
                                `/commerce/govern/limits/${item.limit_id}`,
                              )
                            }
                          ...
                          <button
                            onClick={() =>
                              navigate(
                                `/commerce/govern/limits/${item.limit_id}/edit`,
                              )
                            }
                            className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg"
                          >
        ...
        {filteredLimits.length > 0 && (
          <div className="mt-4 flex items-center justify-between">
            <p className="text-sm text-gray-500">
              Showing{" "}
              <span className="font-medium">{filteredLimits.length}</span>{" "}
              limits
            </p>
          </div>
        )}
