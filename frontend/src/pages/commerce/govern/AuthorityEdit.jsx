...
import { UserCog, Save, ArrowLeft, X, Loader2 } from "lucide-react";
...
const AuthorityEdit = () => {
  ...
  const fetchAuthority = async () => {
    try {
      ...
      if (data.success) {
        setFormData({
          authority_name: data.authority.authority_name || "",
          authority_type: data.authority.authority_type || "approval",
          level: data.authority.level || "manager",
          max_amount: data.authority.max_amount || 0,
          currency: data.authority.currency || "INR",
          scope: data.authority.scope || "department",
          status: data.authority.status || "active",
        });
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        navigate(`/commerce/govern/authority/${authority_id}`);
      } else {
        toast.error(data.detail || "Failed to update authority");
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50" data-testid="authority-edit">
