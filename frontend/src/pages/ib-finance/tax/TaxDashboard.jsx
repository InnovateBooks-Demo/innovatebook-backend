  ...
  const fetchData = async () => {
    try {
      ...
      const dashRes = await fetch(
        `${API_URL}/api/ib-finance/tax/dashboard?period=${periodFilter}`,
        {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Tax determination, calculation & compliance
                </p>
              </div>
            </div>
      ...
      <div className="px-8 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <p
              className={`text-2xl font-bold ${netPayable >= 0 ? "text-amber-600" : "text-green-600"}`}
            >
              {formatCurrency(Math.abs(netPayable))}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              {netPayable >= 0 ? "To be paid" : "Refund due"}
            </p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center gap-3 mb-3">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                {Object.entries(dashboard.by_type?.output || {}).map(
                  ([type, amount]) => (
                    <div
                      key={type}
                    ...
                    >
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium ${getTaxTypeBadge(type)}`}
                      >
                        {type}
                      </span>
            ...
            <div>
              ...
              <div className="space-y-2">
                {Object.entries(dashboard.by_type?.input || {}).map(
                  ([type, amount]) => (
                    <div
                      key={type}
                ...
                <tr key={txn.tax_txn_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-xs text-gray-500">
                      {txn.source_reference_id}
                    </p>
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getTaxTypeBadge(txn.tax_type)}`}
                    >
                      {txn.tax_type}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${getDirectionBadge(txn.direction)}`}
                    >
                      {txn.direction}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right text-gray-900">
                    {formatCurrency(txn.taxable_amount)}
                  </td>
