...
const BillingDetail = () => {
  ...
  const handleApprove = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/ib-finance/billing/${id}/approve`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleIssue = async () => {
    try {
      const token = authService.getToken();
      const response = await fetch(
        `${API_URL}/api/ib-finance/billing/${id}/issue`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
        ...
        <div className="px-8 py-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-sm text-gray-500">
              IB Finance → Billing → {billing.billing_id}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-semibold capitalize ${getStatusBadge(billing.status)}`}
                  >
                    {billing.status}
                  </span>
                </div>
            ...
            <div className="flex gap-2">
              {billing.status === "draft" && (
                <>
                  <button
                    onClick={() => navigate(`/ib-finance/billing/${id}/edit`)}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                  ...
                  <span className="text-sm text-gray-900">
                    {formatDate(billing.created_at)} by {billing.created_by}
                  </span>
                </div>
                {billing.approved_at && (
                  <div className="flex items-center justify-between py-2 border-b border-gray-100">
                    ...
                    <span className="text-sm text-gray-900">
                      {formatDate(billing.approved_at)} by {billing.approved_by}
                    </span>
                  </div>
                )}
                {billing.issued_at && (
                  <div className="flex items-center justify-between py-2 border-b border-gray-100">
                    ...
                    <span className="text-sm text-gray-900">
                      {formatDate(billing.issued_at)} by {billing.issued_by}
                    </span>
                  </div>
                )}
              </div>
            ...
            {billing.status === "issued" && (
              <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-6 text-white">
                ...
                <p className="text-sm text-white/80">
                  This invoice has been issued and a receivable has been
                  created.
                </p>
                <button
