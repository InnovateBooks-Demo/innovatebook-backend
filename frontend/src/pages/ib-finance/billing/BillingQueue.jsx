...
const BillingQueue = () => {
  ...
  const handleApprove = async (billingId) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/ib-finance/billing/${billingId}/approve`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleIssue = async (billingId) => {
    try {
      const token = authService.getToken();
      const response = await fetch(
        `${API_URL}/api/ib-finance/billing/${billingId}/issue`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleCancel = async (billingId) => {
    ...
    try {
      const token = authService.getToken();
      const response = await fetch(
        `${API_URL}/api/ib-finance/billing/${billingId}/cancel`,
        {
          method: "PUT",
          headers: {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Invoice & charge generation from confirmed execution
                </p>
              </div>
            </div>
            <button
              onClick={() => navigate("/ib-finance/billing/create")}
              className="flex items-center gap-2 px-4 py-2 text-blue-700 text-white rounded-lg hover:bg-blue-700 transition-colors"
            >
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredRecords.map((record) => (
                <tr key={record.billing_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${getBillingTypeBadge(record.billing_type)}`}
                    >
                      {record.billing_type}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() =>
                          navigate(`/ib-finance/billing/${record.billing_id}`)
                        }
