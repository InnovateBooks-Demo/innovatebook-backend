...
const BillingEdit = () => {
  ...
  const fetchBillingRecord = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          billing_type: record.billing_type || "milestone",
          ...
          tax_code: record.tax_code || "GST-18",
          line_items: record.line_items?.length
            ? record.line_items
            : [{ description: "", quantity: 1, rate: 0, amount: 0 }],
        });
      } else {
        toast.error("Failed to fetch billing record");
        navigate("/ib-finance/billing");
      }
    } catch (error) {
      toast.error("Failed to fetch billing record");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate(`/ib-finance/billing/${id}`);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50" data-testid="billing-edit">
      <div className="bg-white border-b border-gray-200">
        <div className="px-8 py-6">
          <div className="flex items-center gap-4 mb-4">
            <button
              onClick={() => navigate(`/ib-finance/billing/${id}`)}
              className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg"
            >
              <ArrowLeft className="h-5 w-5" />
            </button>
            <span className="text-sm text-gray-500">
              IB Finance → Billing → Edit
            </span>
          </div>
