...
const CloseDashboard = () => {
  ...
  const fetchPeriods = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const openPeriod =
          periodsData.find((p) => p.status === "open") || periodsData[0];
  ...
  const fetchChecklist = async (period) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/ib-finance/close/checklist?period=${period}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleStartClose = async () => {
    ...
    try {
      const token = authService.getToken();
      const response = await fetch(
        `${API_URL}/api/ib-finance/close/periods/${selectedPeriod.period_id}/start-close`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleAutoClose = async () => {
    ...
    try {
      const token = authService.getToken();
      const response = await fetch(
        `${API_URL}/api/ib-finance/close/auto-close`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        if (data.data.can_close) {
          ...
          toast.warning(
            `Period close blocked - ${data.data.errors.length} issues found`,
          );
        }
  ...
  const handleCompleteClose = async () => {
    ...
    try {
      const token = authService.getToken();
      const response = await fetch(
        `${API_URL}/api/ib-finance/close/periods/${selectedPeriod.period_id}/complete-close`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const autoCloseItems = [
    {
      ...
      icon: Receipt,
      description: "All billing records approved",
    },
    {
      key: "payables_reviewed",
      name: "Payables Review",
      icon: CreditCard,
      description: "All payables 3-way matched",
    },
    {
      key: "depreciation_run",
      name: "Depreciation Run",
      icon: Building2,
      description: "Asset depreciation calculated",
    },
    {
      key: "tax_calculated",
      name: "Tax Calculation",
      icon: Calculator,
      description: "Output/Input tax computed",
    },
    {
      key: "bank_reconciled",
      name: "Bank Reconciliation",
      icon: Building2,
      description: "All accounts reconciled",
    },
    {
      key: "journals_posted",
      name: "Journals Posted",
      icon: BookOpen,
      description: "All draft journals posted",
    },
    {
      key: "trial_balance_reviewed",
      name: "Trial Balance",
      icon: DollarSign,
      description: "Debits = Credits verified",
    },
  ];
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Automated period control & financial finalization
                </p>
              </div>
            </div>
              ...
              {periods.map((period) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${getStatusBadge(period.status)}`}
                    >
                      {period.status}
                    </span>
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span
                      className={`px-3 py-1.5 rounded-full text-sm font-semibold capitalize ${getStatusBadge(selectedPeriod.status)}`}
                    >
                ...
                {autoCloseResult && (
                  <div
                    className={`bg-white rounded-xl border ${autoCloseResult.can_close ? "border-green-200" : "border-amber-200"} overflow-hidden`}
                  >
                    <div
                      className={`px-5 py-4 ${autoCloseResult.can_close ? "bg-green-50" : "bg-amber-50"} border-b ${autoCloseResult.can_close ? "border-green-200" : "border-amber-200"}`}
                    >
                        ...
                        {autoCloseItems.map((item) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <Icon
                                  className={`h-4 w-4 ${passed ? "text-green-600" : "text-red-600"}`}
                                />
                              </div>
                              <div>
                                <p
                                  className={`font-medium text-sm ${passed ? "text-green-800" : "text-red-800"}`}
                                >
                                  {item.name}
                                </p>
                                <p className="text-xs text-gray-500">
                  ...
                  !autoCloseResult && (
                    <div className="bg-white rounded-xl border border-gray-200">
                      ...
                      <div className="divide-y divide-gray-100">
                        {Object.entries(checklistModules).map(
                          ([moduleKey, module]) => (
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 p-5">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <button
                        onClick={() =>
                          navigate(
                            `/ib-finance/close/statements/${selectedPeriod.period}`,
                          )
                        }
