  ...
  const fetchData = async () => {
    try {
      ...
      const accountsRes = await fetch(
        `${API_URL}/api/ib-finance/ledger/accounts`,
        {
      ...
      const journalsRes = await fetch(
        `${API_URL}/api/ib-finance/ledger/journals`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
              ...
              (type) => {
                ...
                return (
                  accountsByType[type]?.length > 0 && (
                    ...
                    >
                      <div
                        className={`px-6 py-3 ${getAccountTypeBadge(type).replace("text-", "bg-").replace("100", "50")}`}
                      >
                          ...
                          {accountsByType[type].map((account) => (
                            ...
                            >
                              ...
                              <td className="px-6 py-3 text-center">
                                <span
                                  className={`px-2 py-1 rounded-full text-xs font-medium ${account.is_active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                                >
                                  {account.is_active ? "Active" : "Inactive"}
                                </span>
                              </td>
                              <td className="px-6 py-3 text-right">
                                <button className="p-2 text-gray-400 hover:text-[#033F99] hover:bg-[rgba(3,63,153,0.08)] rounded-lg">
                ...
                {filteredJournals.map((journal) => (
                  <tr key={journal.journal_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${getStatusBadge(journal.status)}`}
                      >
                        {journal.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() =>
                            navigate(
                              `/ib-finance/ledger/journal/${journal.journal_id}`,
                            )
                          }
