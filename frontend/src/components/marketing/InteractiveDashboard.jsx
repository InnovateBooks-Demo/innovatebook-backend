...
const InteractiveDashboard = ({
  ...
  useEffect(() => {
    ...
    setChartData(
      currentConfig.chartLabels.map(() => Math.floor(Math.random() * 40) + 60),
    );
  }, [type]);
  ...
  return (
    <div className="bg-white rounded-3xl border-2 border-slate-200 overflow-hidden shadow-xl hover:shadow-2xl transition-shadow">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/80 text-sm">
                Interactive demo • Data updates live
              </p>
            </div>
          </div>
          <button
      ...
      <div className="px-6 pb-6">
        <div
          className={`p-4 rounded-2xl bg-gradient-to-r ${config.lightGradient} border ${colors.border}`}
        >
          <div className="flex items-center justify-between mb-4">
            <p className="font-semibold text-slate-700">Trend Analysis</p>
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <RefreshCw
                className={`h-4 w-4 ${isPlaying ? "animate-spin" : ""}`}
              />
              <span>Live</span>
            </div>
          </div>
