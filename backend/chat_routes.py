...
# )
# from app_state import get_database, get_current_user
# import os
...
# =======================
# # WEBSOCKET (SECURED) - LEGACY - Using workspace_router instead
# # =======================
# @router.websocket("/ws/{user_id}")
# async def websocket_endpoint(websocket: WebSocket, user_id: str, token: str = Query(None)):
#     """
#     WebSocket endpoint for real-time messaging + signaling
#     Client connects: /api/chat/ws/{userId}?token=JWT
#     """
#     if not token:
...
#         token_user_id = payload.get("sub") or payload.get("user_id") or payload.get("id")
...
#                 ws_message = {
#                     "type": WSMessageType.USER_TYPING,
#                     "data": {"user_id": user_id, "channel_id": data["channel_id"]},
#                     "timestamp": datetime.now(timezone.utc).isoformat()
#                 }
#                 await manager.broadcast_to_channel(data["channel_id"], ws_message)
#
