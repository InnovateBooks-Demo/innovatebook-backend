...
async def get_current_user(
    ...
    try:
        ...
        # 🔥 VERY IMPORTANT: attach org_id for downstream dependencies
        user["org_id"] = org_id
...
#         # Create access token
#         access_token = create_access_token(
#             data={"sub": str(user["user_id"]), "org_id": tenant_id}
#         )
...
async def login(request: LoginRequest, db=Depends(get_db)):
    try:
        ...
        # ---- ORG RESOLUTION (THIS WAS BROKEN BEFORE) ----
        ...
        if not org_mappings:
            raise HTTPException(
                status_code=403,
                detail="No active organization found for this user"
            )
        ...
        logger.warning(f"🔐 Login HTTPException: {e.detail}")
        raise
    except Exception as e:
...
async def select_tenant(request: TenantSelectionRequest, db = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    try:
        ...
        access_token = create_access_token(
            data={"sub": current_user["user_id"], "tenant_id": request.tenant_id}
        )
