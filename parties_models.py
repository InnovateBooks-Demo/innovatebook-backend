...
class PartyBase(BaseModel):
    """Base fields that come FROM the client (request)"""
    display_name: str
    legal_name: str
    party_sub_type: Optional[str] = None
    country_of_registration: str
...
class PartyBaseStored(PartyBase):
    """Fields always present in DB/response (server owned)"""
    party_category: PartyCategory
...
#     last_modified_by: Optional[str] = None
#     last_modified_at: Optional[datetime] = None
class Customer(CustomerCreate, PartyBaseStored):
    party_id: str
    customer_id: str
    org_id: str
    created_by: str
