...
async def get_organizations_overview(token_payload: dict = Depends(require_super_admin)):
    ...
    try:
        ...
        orgs = await get_db().organizations.find({}, {"_id": 0}).to_list(None)
        ...
        for org in orgs:
            ...
            # Get user counts
            total_users = await get_db().enterprise_users.count_documents({"org_id": org_id})
            active_users = await get_db().enterprise_users.count_documents({"org_id": org_id, "is_active": True})
            inactive_users = total_users - active_users
            
            # Get subscription details
            subscription = await get_db().subscriptions.find_one({"org_id": org_id}, {"_id": 0})
            
            # Get activity metrics (last 30 days)
            thirty_days_ago = datetime.now(timezone.utc) - timedelta(days=30)
            
            # Count data created in last 30 days
            customers_count = await get_db().customers.count_documents({"org_id": org_id})
            invoices_count = await get_db().invoices.count_documents({"org_id": org_id})
            leads_count = await get_db().commerce_leads.count_documents({"org_id": org_id})
...
async def get_platform_growth(token_payload: dict = Depends(require_super_admin)):
    ...
    try:
        ...
        # Get user growth
        users = await get_db().enterprise_users.find(
...
async def get_organization_details(org_id: str, token_payload: dict = Depends(require_super_admin)):
    ...
    try:
        org = await get_db().organizations.find_one({"org_id": org_id}, {"_id": 0})
        if not org:
            raise HTTPException(status_code=404, detail="Organization not found")
        
        # Get all users
        users = await get_db().enterprise_users.find({"org_id": org_id}, {"_id": 0, "password_hash": 0}).to_list(None)
        ...
        # Get all data counts
        data_metrics = {
            "customers": await get_db().customers.count_documents({"org_id": org_id}),
            "vendors": await get_db().vendors.count_documents({"org_id": org_id}),
            "invoices": await get_db().invoices.count_documents({"org_id": org_id}),
            "bills": await get_db().bills.count_documents({"org_id": org_id}),
            "leads": await get_db().commerce_leads.count_documents({"org_id": org_id}),
            "employees": await get_db().employees.count_documents({"org_id": org_id})
        }
...
async def get_user_activity(token_payload: dict = Depends(require_super_admin)):
    ...
    try:
        total_users = await get_db().enterprise_users.count_documents({"is_super_admin": False})
        active_users = await get_db().enterprise_users.count_documents({"is_super_admin": False, "is_active": True})
        
        # Get users by org
        orgs = await get_db().organizations.find({}, {"_id": 0, "org_id": 1, "org_name": 1}).to_list(None)
        
        user_distribution = []
        for org in orgs:
            count = await get_db().enterprise_users.count_documents({"org_id": org["org_id"]})
